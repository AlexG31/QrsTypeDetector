...
class QrsTypeDetector(object):
    def __init__(self, result_file_path, record_ID, fs = 500.0):
        ...
        # Load diagnosis progress
        self.diag_progress = None
        tmp_file_name = '/tmp/diag_progress.json'
        if os.path.exists(tmp_file_name):
            with open(tmp_file_name, 'r') as fin:
                self.diag_progress = json.load(fin)
        if self.diag_progress is None:
            self.diag_progress = dict()
    ...
    def GetQrsSum(self, raw_sig, q_index, r_index, s_index, MaxSearchLengthMs = 10):
        ...
        new_pos_list.append(np.argmin(raw_sig[left_bound:right_bound + 1]) + left_bound)
        
        # r
        left_bound = q_index
        # left_bound = max(q_index, r_index - R_MaxSearchIndexLen)
        # right_bound = min(s_index, r_index + R_MaxSearchIndexLen)
        right_bound = s_index
        left_bound = int(left_bound)
        right_bound = int(right_bound)
        ans += np.max(raw_sig[left_bound:right_bound + 1])
        new_pos_list.append(np.argmax(raw_sig[left_bound:right_bound + 1]) + left_bound)
    ...
    def GetQrsDegree(self, q_index, r_index, s_index, MaxSearchLengthMs = 10):
        ...
        if r_index not in self.diag_progress[self.record_ID]['AVF']:
            new_pos_list, qrs_type = self.GetQrsType(self.sig_data['aVF'], q_index, r_index, s_index, MaxSearchLengthMs = MaxSearchLengthMs, debug_plot = False)
            ...
            new_pos_list, qrs_type = self.GetQrsType(raw_sig, q_index, r_index, s_index, MaxSearchLengthMs = MaxSearchLengthMs)
